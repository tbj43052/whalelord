...
**Loading screen and shared sprites** — `whale-sprite.js` holds the procedural Whale Lord and JEET fish drawings (used by the game and the loader). `loading.js` + `loading.css` are the "Deep Dive" loading screen: a blurred ocean void with drifting sunbeams and a distant gold-trimmed wreck, the Whale Lord diving nose-first with a bubble and gold-glitter wake, and a gold-bezel progress bar whose fill pushes JEET fish along. `WhaleLoader.start()`, `set(0..1)`, `done()`. The landing page drives it from the six image loads; the game page from font, sprite and reef build steps. It stays up at least 1.4 s and auto-dismisses after 12 s no matter what.
