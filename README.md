...
4. **Bottom bar** — absolute bottom, full width, `padding: 12px 28px`, grid `1fr auto 1fr`, background `rgba(5,11,20,.85)`, `backdrop-filter: blur(12px)`, `border-top: 1px solid rgba(212,175,55,.35)`.
   - Left: "OFFICIAL WEBSITE | DIVE INTO DOMINANCE", Nunito 800 12px, letter-spacing .14em, white; the "|" is gold with 6px side margins.
...
Other copy: header title "THE WHALE LORD"; link box "$WHALELORD"; bottom bar "OFFICIAL WEBSITE | DIVE INTO DOMINANCE", "CONTRACT:", "Copy CA", "Copied!"; panel eyebrow "About"; "Close".
...
**Loading screen** — `loading.js` + `loading.css` mount the "The Whale Lord Loading Screen" component (navy starfield, gold-beveled title, gold-bezel bar with sheen, Cinzel percentage, "LOADING THE DEEP..." caption, anchor mark) as the full-screen overlay `#loading-screen`. Rules: `requestAnimationFrame` drives the bar width and the percentage from 1% to 100% over 1.8 s, advancing at most one whole number per frame so every integer renders in order; if assets finish early the full run still plays; if they are slow the count holds at 99% until `WhaleLoader.done()`, then steps to 100%; at 100% it holds 200 ms, fades over 0.5 s, then is removed. The landing page calls `done()` when its six images are in; the game page after fonts, sprites and reef are built. A 12 s safety timer always releases it. `whale-sprite.js` holds the shared Whale Lord and JEET fish drawings used by the game.

**Mobile** — both pages use `100dvh`, `viewport-fit=cover` and safe-area padding on the bottom bar. Under 720px the header stacks (title, then a compact nav row) and the contract address is shortened on screen while the full address is still what Copy CA copies. Landing page: the wide artwork is cropped to its centre on phones, so the five story bubbles move into a strip above the contract bar (`#orb-strip`, handled by `layoutOrbs()` in index.html) and move back into the scene on wider screens. Game page: portrait phones get a "turn your phone sideways" hint, touch devices see drag/hold/tap instructions instead of the keyboard list, the HUD compacts, and on short landscape screens (under 540px tall) the contract bar becomes static so it never covers the game.

**Site icons** — `favicon.ico` (16/32/48 PNG-in-ICO) at the root plus `assets/icons/` (favicon-16/32/48, apple-touch-icon 180, icon-192/512) and `site.webmanifest`, all cropped from the Whale Lord "sunset drive" artwork (face, smirk, cigar). Both pages link them in `<head>`. To regenerate from a new picture, re-run the crop script (PowerShell + System.Drawing; crop centre 53%/35%, size 46% of the image).
