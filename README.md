...
**Live site** — https://tbj43052.github.io/whalelord/ (GitHub Pages, repo https://github.com/tbj43052/whalelord). GitHub publishes the `gh-pages` branch, so after committing on `main` push both: `git push origin main && git push origin main:gh-pages`. The hosted copy has no backend, so the leaderboard runs in browser-only mode until the Node server is deployed and `API_BASE` in `game.js` points at it.
